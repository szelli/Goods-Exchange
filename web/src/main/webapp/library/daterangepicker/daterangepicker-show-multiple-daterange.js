...
!function ($, moment) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        this.parentEl = $(element).parent();

        //element that triggered the date range picker
        this.element = $(element);

        //create the picker HTML object
        var DRPTemplate = '<div class="daterangepicker dropdown-menu dropdown_'+ this.parentEl.attr('dropdown') +'">' +
    ...
    DateRangePicker.prototype = {
        ...
        setOptions: function(options, callback) {
            ...
            this.opens = 'right';
            if (this.parentEl.hasClass('pull-right')){
                this.opens = 'left';
            }
            ...
            if (typeof options.dateRanges === 'object'){
                for(var d=0; d<options.dateRanges.length; d++){
                    options.dateRanges[d].startDate = moment(options.dateRanges[d].startDate);
                    options.dateRanges[d].endDate = moment(options.dateRanges[d].endDate);
                }
            ...
            if (this.container.hasClass('dropdown_false')) {
                this.container.removeClass('dropdown-menu').removeClass('opensright').removeClass('opensleft');
            }
            this.updateView();
        ...
        toggle: function (e) {
            if (this.container.hasClass('dropdown_true')) {
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    if(this.container.hasClass('dropdown_false')){
                        var isinrange_array = [];
                        var isinrangestart_array = [];
                        var isinrangeend_array = [];
                        var isinrange = false;
                        var isinrangestart = false;
                        var isinrangeend = false;
                        for(var d=0; d<this.dateRanges.length; d++){
                            isinrange_array.push((calendar[row][col] > this.dateRanges[d].startDate && calendar[row][col] < this.dateRanges[d].endDate));
                            isinrangestart_array.push(calendar[row][col].isSame(this.dateRanges[d].startDate.format('YYYY-MM-DD')));
                            isinrangeend_array.push(calendar[row][col].isSame(this.dateRanges[d].endDate.format('YYYY-MM-DD')));
                        }
                        for(var r=0; r<isinrange_array.length; r++){
                            if(isinrange_array[r]){
                                isinrange = true;
                                break;
                            }
                        }
                        for(var r=0; r<isinrangestart_array.length; r++){
                            if(isinrangestart_array[r]){
                                isinrangestart = true;
                                break;
                            }
                        }
                    ...
                    if ((minDate && calendar[row][col].isBefore(minDate, 'day')) || (maxDate && calendar[row][col].isAfter(maxDate, 'day'))) {
                        cname = ' off disabled ';
                    } else if (calendar[row][col].format('YYYY-MM-DD') == selected.format('YYYY-MM-DD') && !this.container.hasClass('dropdown_false')) {
                        cname += ' active ';                        
